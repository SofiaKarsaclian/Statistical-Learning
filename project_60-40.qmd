...
Below we see a list of all the features that contribute to the XGBoost model's performance sorted from highest to lowest (gain). We see that radius_worst and concave.points_mean have the highest influence on the models classification performance. Other features contribute as well but are not nearly as important as these two. None of the features is among those being insignificantly related to the target variable diagnosis.
...
We initialized a baseline lasso classifier with a penalty of 0.01 and fitted this initial model into a 10-fold cross-validation of the training data. Even with this initial model, the ROC-AUC plot blow shows that the performance is excellent (𝜇= 0.998). Recall, however, is not as good. Given that it is the most important metric in cancer detection, we tried to improve it through hyperparameter tuning.
...
We tune the model's $l_1$ penalty by creating a grid of 100 sensible values to try with `grid_regular()`. We learn that the value of $l_1$ that yields the best recall (𝜇 = 0.993) is 0.00059, and that this value also yields high ROC-AUC.
...
Finally, we fit the lasso with the tuned penalty to the train data, and evaluate the performance on the test data. The model performs really well, with a high capability of distinguishing classes (ROC-AUC = 0.98), and a high recall (0.95) which means that the model is effective at capturing most positive cases, minimizing false negatives.

With the penalty parameter set at 0.00059, the majority of coefficients were effectively regularized to zero, resulting in the selection of 13 features considered most important for the classification.<br> Among these, the variables that contributes the most to classify the breast mass as malignant are all those related to fractal dimension. The larger (mean and worst) and the more variations (SE) there are in fractal dimension, the more likely for it to be classified as malignant.<br> On the other hand, higher values of smoothness (and variation in the smoothness), symmetry and concavity are associated with a more likely benign classification. These features seem to be characteristics observed in non-cancerous masses. Notably, smoothness (SE) and texture (SE), as well as fractal dimension (SE and mean) are among the features initially found to be insignificantly related to the target variable in a simple logistic regression.
