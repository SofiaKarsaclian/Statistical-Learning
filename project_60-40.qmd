...
Inspecting the correlation between the features and the target variable revealed low correlation values (smaller than 0.5) for the features presented in Table 1. A simple logistic regression further revealed a statistically insignificant relation between the target variable and the features shown in Table 2.
...
Selecting the hyperparameter settings that maximized recall averaged across all folds, displayed in Table 3, we fit the model to the test data and received the model's test performance, which can be seen in Table 4.<br>
...
In Table 5 we see a list of all the features that contribute to the XGBoost model's performance sorted from highest to lowest (gain). We see that `radius_worst` and `concave.points_mean` have the highest influence on the models classification performance. Other features contribute as well but are not nearly as important as these two. None of the features are among the ones, identified above, to be insignificantly related to the target variable, diagnosis.
...
kable(xgb.importance(model = bst))
```

## Lasso Regression

We initialized a baseline lasso classifier with a penalty of 0.01 and fitted this initial model into a 10-fold cross-validation of the training data. Even with this initial model, the model metrics and ROC-AUC plot blow show that the performance is excellent (𝜇= 0.998). Recall, however, is not perfect yet. Given that it is the most important metric in cancer detection, we tried to improve it through hyperparameter tuning.
...
```{r Lasso Regression Fig, echo = FALSE, results = 'asis'}
...
We tuned the model's $l_1$ penalty by creating a grid of 100 sensible values to try with `grid_regular()`. We learned that the value of $l_1$ that yields the best recall (𝜇 = 0.993) is 0.00059 (Table 7 & 8), and that this value also yields high a ROC-AUC measure.

*Figure 4*

```{r Tuning Lassos Penalty Parameter - Fig}
#| echo: false
...
```{r Tuning Lassos Penalty Parameter - Tab 5}
#| echo: false
...
```{r Tuning Lassos Penalty Parameter - Tab 6}
#| echo: false
...
Finally, we fitted the lasso with the tuned penalty to the training data, and evaluated the performance on the test data. The model performs really well, with a high capability of distinguishing classes (ROC-AUC = 0.98), and a high recall (0.95), which means that the model is effective at capturing most positive cases, minimizing false negatives.

With the penalty parameter set to 0.00059, the majority of coefficients were effectively regularized to zero, resulting in the selection of 13 features, shown in Table 9, considered to be most important for the classification.<br>
Among these, the variables that contribute the most to classifying the breast mass as malignant are all those related to fractal dimension. The larger (mean and worst) and the more variations (SE) there are in fractal dimension, the more likely for it to be classified as malignant.<br>
On the other hand, higher values of smoothness (and variation in smoothness), symmetry and concavity are associated with a more likely benign classification. These features seem to be characteristics observed in non-cancerous masses. Notably, smoothness (SE) and texture (SE), as well as fractal dimension (SE and mean) are among the features initially found to be insignificantly related to the target variable in a simple logistic regression.

*Table 9*

```{r Lasso Performance - Tab 8}
#| echo: false
...
```{r Conc Fig, echo = FALSE, results = 'asis'}
