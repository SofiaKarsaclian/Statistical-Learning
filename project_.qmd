...
train_test = 0.8 # define how much of the data is used as training/test data
set.seed(seed)
...
We are going to compare two different models for this project: eXtreme Gradient Boosting (XGBoost) and lasso regression. Hence, we do not need to perform normalization and feature selection ourselves.<br>
...
Hence, given the small size of the dataset, dropping features yields little in terms of computing speed and omits data that is nevertheless potentially useful for XGBoost and and not detrimental to lasso regression.<br>

Following Emil Hvitfeldt's and Julia Silge's [Supervised Machine Learning for Text Analysis in R](https://smltar.com/), we fit both models using the tidymodels R package. After cross-validating different distributions we split our data into 80% training and 20% testing data. This resulted in the best model performances. To optimize hyperparameter settings we used 10-fold cross validation and the recall metric.<br>
...
XGBoost allows for many tunable hyperparameters. The more of these we tune, the more we increase computing time for model fitting. Thus, we started out with a baseline approach. We only tuned the number of decision trees and used the xgboost R package's default values for the other parameters. We then also tuned the depth of the trees, further increasing an already good performance. Moreover, we tried to optimize performance by tuning all hyperparameters, but after performing 10-fold cross validation on 25,000 models - a time consuming process - we could not observe any additional improvements. It seems that the default values for learning rate, loss reduction, and minimum node weight hit a sweet spot.
...
Below we see the model performance on the test data.<br>
Once for the model where we only tuned the number of trees and their respective depth (optimal):

*Table 3*

```{r Print XGBoost Model 1, echo = FALSE, results = 'asis'}

kable(collect_metrics(xg_trees_depth))
```

And for the model where we tuned all hyperparameters:

*Table 4*

```{r Print XGBoost Model 2, echo = FALSE, results = 'asis'}

kable(collect_metrics(xg_trees_depth_n_rate_reduction))
metrics_xgboost <- collect_metrics(xg_trees_depth)
```

Of course, extensively tuning all parameters will eventually yield the best model. This takes a lot of resources though, and due to the fact that we already fitted a model on the default values we did not include them in the grid search for this model. We see that hyperparameter settings for loss reduction, learning rate, and minimum node weight below or above the default values do not increase model performance according to recall.<br>

The following hyperparameter settings yielded the best performing XGBoost model:<br>
...
```{r Parameters of XGBoost, echo = FALSE, results = 'asis'}
#| eval: false

xg_trees_depth$.workflow
```

## Lasso Regression

We initialized a baseline lasso classifier with a penalty of 0.01 and fitted this initial model into a 10-fold cross-validation of the training data. Even with this initial model, the ROC-AUC plot blow shows that the performance is excellent (𝜇= 0.993). Recall, however, is slightly lower. Given that it is the most important metric in cancer detection, we tried to improve it through hyperparameter tuning.

*Table 6*

```{r Lasso Regression Table, echo = FALSE, results = 'asis'}
...
```{r Lasso Regression Plot, echo = FALSE, results = 'asis'}
...
```{r Tuning Lassos Penalty Parameter - Plot}
#| echo: false
...
```{r Tuning Lassos Penalty Parameter - Table 6}
#| echo: false
...
```
*Table 7*
```{r Tuning Lassos Penalty Parameter - Table 7}
#| echo: false
...
We learn that the value of $l_1$ that yields the best recall (𝜇= 0.96) is 0.0023645, and that this value also yields high roc_auc.
...
With a penalty parameter set at 0.0023645, the majority of coefficients were effectively regularized to zero, resulting in the selection of 14 features considered most important for the classification.<br>
Among these, the variable that contributes the most to classify the breast mass as malignanat is the compactness SE: the more changes or variations in the mass' compactness, the more likely for it to be classified as malignant. To a lesser extent, mean fractal dimension also contributes to a malignant classification.<br>
