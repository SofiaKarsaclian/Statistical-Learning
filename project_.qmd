...
A simple barplot of the target variable, diagnosis, reveals that the dataset is unbalanced. About 37 percent of all observations are malignant, whereas 63 percent are benign.
...
corr_df[corr_df$correlation < 0.5,]
...
logreg_df[logreg_df$significant == "No",]
```
...
Following Emil Hvitfeldt's and Julia Silge's 'Supervised Machine Learning for Text Analysis in R' we fit both models using the tidymodels R package. After testing different distributions we split our data into 80% training and 20% testing data. This resulted in the best model performances. To choose the best hyperparameter settings we used 10-fold cross validation and the recall metric.<br>
Since it is most important to avoid false negatives when dealing with cancer data we weighed a model's recall more heavily than its F1-score, precision, or roc-auc measure.

## eXtreme Gradient Boosting (XGBoost)

With XGBoost there are many tunable hyperparameters. With each hyperparameter we would tune the model fitting would take more and more time so we started out with a baseline approach where we only tuned the number of decision trees and used the xgboost R package's default values for the other parameters. Tuning the depth of the trees also, further increased an already good performance. We tried to further improve performance by tuning all hyperparameters but after performing 10-fold cross validation on 25,000 models, a time consuming process, we could not further improve performance. It seems that the default values for learning rate, loss reduction, and minimum node weight hit a sweet spot.
...
Strikingly, both models yield almost the exactly same performance, both being very effective in the binary classification. It's possible that both models converge to similar solutions during training and hyper parameter tuning. Overall, we managed to achieve a high level of recall with both models, which was our goal to minimize false negatives in cancer detection. 

In addition to their performance metrics, it's important to consider the strengths and weaknesses of each model.  As above mentioned, both models perform feature selection (through different mechanism) which helps identify which are the most important variables. While Lasso's coefficients provide interpretability, XGBoost is more of a black box. This also means however that XGBoost is more flexible and has the power capture nonlinear relationships, while Lasso cannot. This, however, does not seem to be important for this application. Finally, as shown above, Lasso is more computational efficient than XGBoost.
