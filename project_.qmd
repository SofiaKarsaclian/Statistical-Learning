...
model_nr = 3 # currently set to the 'best' model (if set to 3 chunk takes a long time to run)
...
saveRDS(xg_wf_final, 'test.RDS')

xg_final = last_fit(
  readRDS('test.RDS'), training_split, metrics = metrics
  )
```
...
Below we see the model performance on the test data.<br> Once for the model where we only tuned the number of trees and their respective depth (optimal):
