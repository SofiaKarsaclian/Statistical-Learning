...
target_metric = 'roc_auc' # measure of model performance
...
set.seed(seed)
training_split = initial_split(data, prop = 0.6, strata = diagnosis)
...
We tune the model's $l_1$ penalty by creating a grid of 100 sensible values to try with `grid_regular`. We learn that the value of $l_1$ that yields the best recall (𝜇= 0.993) is 0.00059, and that this value also yields high roc_auc.
