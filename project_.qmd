...
Following Emil Hvitfeldt's and Julia Silge's 'Supervised Machine Learning for Text Analysis in R' we fit both models using the tidymodels R package. We split our data into 80% training and 20% testing data and used 10-fold cross validation to asses model performance.
...
With XGBoost there are many tunable hyperparameters. We started by fitting a model where we only tuned the number of decision trees used to get a first impression. Following promising performance metrics we also tuned the other hyperparameters.<br>
