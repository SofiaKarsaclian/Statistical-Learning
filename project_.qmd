...
XGBoost allows for many tunable hyperparameters which's inclusion exponentially increases computing time for model fitting. Thus, we started out with a baseline approach. We only tuned the number of decision trees and used xgboost's R package default values for the other parameters. We then tuned the depth of the trees, further increasing an already good performance. Moreover, we tried to optimize performance by tuning all hyperparameters, but after performing 10-fold cross validation on 25,000 models, a time consuming process, we could not observe any additional improvements. It seems that the default values for learning rate, loss reduction, and minimum node weight hit a sweet spot.
...
kable(metrics_xgboost <- collect_metrics(xg_trees_depth))
```
